...
[claude.ai](https://claude.ai) にアクセスし、比較したい2つのWord文書（.docx）をチャット画面にドラッグ＆ドロップしてください。
...
- **Pro/Teamプラン推奨**: 無料プランでも利用可能ですが、長い文書の場合はPro以上のプランが安定します
...
Cursorでも同様に、ファイルを指定して依頼すれば対比表を生成できます。
...
はい。ブラウザでclaude.aiにアクセスして、ファイルをドラッグ＆ドロップするだけで使えます。環境構築は不要です。
