...
#### 手順1: スキルのzipファイルをダウンロード

このリポジトリから `skills` フォルダをzipファイルとしてダウンロードします。

**方法A: GitHubからダウンロード**
1. このリポジトリのページで「Code」ボタンをクリック
2. 「Download ZIP」を選択
3. ダウンロードしたzipを解凍
4. `skills` フォルダを再度zipに圧縮（`skills.zip` など）

**方法B: 直接圧縮（ローカルにクローン済みの場合）**
```bash
# リポジトリのルートで実行
...
2. 先ほど作成した `skills.zip` ファイルを選択してアップロード
...
| **スキル更新** | 新バージョンがリリースされたら、再度zipをアップロードして更新してください |
...
1. zipファイルの構造を確認（`skills/comparison-table-v2/SKILL.md` が存在するか）
2. SKILL.md の形式が正しいか確認
...
設定画面で古いスキルを削除し、新しいzipファイルをアップロードしてください。
